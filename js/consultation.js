/**
 * Apex Diurnal - Commercial Solar Consultation & Corporate RFQ Modal Controller
 */
(function initConsultationModal() {
  function setup() {
    ...
    function setupProvinceCityDropdowns(provinceSelectId, citySelectId, errCityId) {
      const pSelect = document.getElementById(provinceSelectId);
      ...
      function updateCities(preserve) {
        ...
        const list = getCitiesForProvince(prov);

        cSelect.innerHTML = '<option value="">Select City / Municipality</option>';
    ...
    function validateRfqStep1() {
      ...
      if (!regType || !regType.value) {
        showFieldError('rfq_business_registration_type', 'Please select your business registration type.');
        valid = false;
      }
      return valid;
    }
    ...
    function validateRfqStep2() {
      ...
      if (!street || street.value.trim().length < 5) {
        showFieldError('rfq_street', 'Please provide a complete street address (minimum 5 characters).');
        valid = false;
      }
      if (!province || !province.value) {
        showFieldError('rfq_province', 'Please select an installation province.');
        valid = false;
      }
      if (!city || !city.value) {
        showFieldError('rfq_city', 'Please select an installation city / municipality.');
        valid = false;
      } else if (province && province.value) {
        const allowedCities = getCitiesForProvince(province.value);
        if (allowedCities.length > 0 && !allowedCities.includes(city.value)) {
      ...
      if (!postal || !/^\d{4}$/.test(postal.value.trim())) {
        showFieldError('rfq_postal', 'Please enter a valid 4-digit postal code.');
        valid = false;
      }
      return valid;
    }
